...
def check_and_delete(fp, wait=0):
    ...
    thread.start_new_thread(run, (wait))
...
def getCache(msg, resident=False):
    """ 获取缓存的语音 """
    md5 = hashlib.md5(msg.encode('utf-8')).hexdigest()
    prefix = 'r-' if resident else 'c-'
    mp3_cache = os.path.join(constants.TEMP_PATH, prefix + md5 + '.mp3')
    wav_cache = os.path.join(constants.TEMP_PATH, prefix + md5 + '.wav')
...
def saveCache(voice, msg, resident=False):
    """ 获取缓存的语音 """
    foo, ext = os.path.splitext(voice)
    md5 = hashlib.md5(msg.encode('utf-8')).hexdigest()
    prefix = 'r-' if resident else 'c-'
    target = os.path.join(constants.TEMP_PATH, prefix + md5 + ext)
    shutil.copyfile(voice, target)
    return target


def lruCache(resident=False):
    """ 清理最近未使用的缓存 """

    def run(prefix):
        if config.get('/lru_cache/enable', True):
            days = config.get('/lru_cache/days', 7)
            cmd = 'find . -name "%s*.mp3" -atime +%d -exec rm {} \\;' % prefix, days
            subprocess.run(cmd, cwd=constants.TEMP_PATH, shell=True)
    ...
    if resident is None:
        ...
        prefix = 'c-'
    thread.start_new_thread(run, (prefix))
